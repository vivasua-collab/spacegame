...
| Сериализация (systemMap excl.) + **компактный формат v3 (R-29, ленивые залежи)** | `game-store.ts:serializeGameState` + `src/lib/save-format-v3.ts` (декодер v2 — для старых сейвов) | ✅ |
...
> генерации); флаг `depositsMaterialized` исключает дубли; сейвы v1/v2
> (запечённые) помечаются при загрузке. Формат v3: словарь `galaxy.dict`
...
>
> **R-28 (2026-08-31) — формат сейва v2 (история, декодер жив):** залежи и
...
> 5.3 → 2.3 МБ. Обратная совместимость: сейвы без `fmt:2` читаются
> как раньше (`expandSaveV2` — путь загрузки старых сейвов).
