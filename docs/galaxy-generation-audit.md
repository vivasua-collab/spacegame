# Аудит генерации галактики — код vs документация

> **Дата обновления:** 2026-05-03 (v3 — после рефакторинга генератора на оркестратор + модули)
> **Цель:** Полная документация текущей реализации генерации, отражающая архитектуру оркестратора, все применённые исправления и выявленные расхождения.
> **Спецификация:** `docs/30-planets.md` (единый источник истины), дополнительно `docs/20-stars.md`, `docs/10-galaxy.md`
> **Код:** `src/galaxy/` (оркестратор + 5 модулей + gen-context + hex-grid)

---

## Содержание

1. [Статус исправлений G-01..G-31](#1-статус-исправлений-g-01g-31)
2. [Архитектура генерации (оркестратор + модули)](#2-архитектура-генерации)
3. [Типы планет — радиус, гравитация, размер сетки](#3-типы-планет--радиус-гравитация-размер-сетки)
4. [Температура планеты](#4-температура-планеты)
5. [Атмосфера — вероятности типов](#5-атмосфера--вероятности-типов)
6. [Внутренние противоречия в спецификации](#6-внутренние-противоречия-в-спецификации)
7. [Жизнь на планетах](#7-жизнь-на-планетах)
8. [Ресурсы](#8-ресурсы)
9. [Орбитальные и атмосферные слоты](#9-орбитальные-и-атмосферные-слоты)
10. [Звёзды и планетные системы](#10-звёзды-и-планетные-системы)
11. [Спиральная структура галактики](#11-спиральная-структура-галактики)
12. [Jump Points и связность](#12-jump-points-и-связность)
13. [Осознанные отклонения от спецификации](#13-осознанные-отклонения-от-спецификации)
14. [Нереализованные возможности из спецификации](#14-нереализованные-возможности-из-спецификации)
15. [Сводная таблица текущих расхождений](#15-сводная-таблица-текущих-расхождений)

---

## 1. Статус исправлений G-01..G-32

Все критические и значительные баги, обнаруженные в v1/v2/v3 аудита, исправлены в коде:

| ID | Описание | Статус | Файл |
|-----|----------|--------|------|
| G-01 | lifeChance не соответствует спец. | ✅ ИСПРАВЛЕНО | planet-types.ts: lifeChance обновлены; LIFE_LEVEL_WEIGHTS добавлены |
| G-02 | Парниковый эффект не координирован с атмосферой | ✅ ИСПРАВЛЕНО | generate-planets.ts: атмосфера генерируется ДО температуры; calculatePlanetTemperature принимает реальный Atmosphere |
| G-03 | Температура использует starDef, не Star | ✅ ИСПРАВЛЕНО | generate-systems.ts + generate-planets.ts: generatePlanet получает primaryStar (Star), не starDef |
| G-04 | Размер ±1 вар. позволяет радиусы ЗА ПРЕДЕЛАМИ спец. | ✅ ИСПРАВЛЕНО | generate-planets.ts: радиус из PLANET_TYPE_RADIUS; размер из getSizeFromRadius() |
| G-05 | Диск не генерируется; diskRng не используется | ✅ ИСПРАВЛЕНО | generate-positions.ts: diskRng используется, diskCount=20% |
| G-06 | Размер сетки не из R⊕ | ✅ ИСПРАВЛЕНО | planet-types.ts: getSizeFromRadius() из R⊕ |
| G-07 | Атмосферные вероятности приблизительные | ✅ ИСПРАВЛЕНО | generate-planets.ts: условные вероятности точно пересчитаны из §2.4 |
| G-08 | Gas Giant: inert/toxic переставлены | ✅ ИСПРАВЛЕНО | generate-planets.ts: inert=5%, toxic=4% |
| G-09 | Уровни жизни: единое распределение | ✅ ИСПРАВЛЕНО | generate-planets.ts: LIFE_LEVEL_WEIGHTS per-type |
| G-10 | Условия для жизни не проверяются | ✅ ИСПРАВЛЕНО | generate-planets.ts: температура, атмосфера, toxic ограничения |
| G-11 | Toxic атмосфера полностью блокирует жизнь | ✅ ИСПРАВЛЕНО | generate-planets.ts: toxic позволяет microbes (экстремофилы) |
| G-12 | Спутник звезды может стать спецтипом | ✅ ИСПРАВЛЕНО | generate-systems.ts: selectCompanionStar только из main sequence |
| G-13 | Орбитальный радиус не учитывает тип двойной системы | ✅ ИСПРАВЛЕНО | generate-planets.ts: BINARY_CLOSE→min 1 AU, BINARY_WIDE→max 30 AU |
| G-14 | Ядро галактики: ~30% вместо ~15% | ✅ ИСПРАВЛЕНО | generate-positions.ts: bulgeFraction=0.15 |
| G-15 | Орбитальные слоты зависят от типа, не от размера | ✅ ИСПРАВЛЕНО | generate-planets.ts: ORBIT_SLOTS_BY_SIZE для не-ГГ |
| G-16 | selectPlanetType использует HZ вместо Snow Line | ✅ ИСПРАВЛЕНО | generate-planets.ts: snowLine + 4 зоны (innerHZ/inHZ/betweenHZAndSnow/beyondSnow) |
| G-17 | Астероидные поля не учитывают ГГ и двойную систему | ✅ ИСПРАВЛЕНО | generate-systems.ts: +1 за ГГ, +1 за двойную |
| G-18 | Коэффициент 278 вместо 278.5 | ✅ ИСПРАВЛЕНО | generate-planets.ts: 278.5 |
| G-19 | Вариация звёзд ±20% вместо subclass-based | ✅ ИСПРАВЛЕНО | generate-systems.ts: G-32 fix — ±15% для M и L, R из массы, T из Стефана-Больцмана |
| G-24 | PRNG derive() использует слабый DJB2 | ✅ ИСПРАВЛЕНО | prng.ts: 4 независимых хеша (FNV-1a + Murmur2 + Murmur3) |
| G-25 | Rocky/Oceanic max radius расширен для "large" сетки | 🟡 Осознанное отклонение | planet-types.ts:30,33 |
| G-27 | Орбитальный радиус масштабируется от hzCenter | 🟡 Осознанное отклонение | generate-planets.ts: hzCenter |
| G-29 | Gas giant density min поднят для мин. гравитации | 🟡 Осознанное отклонение | planet-types.ts:20 |
| G-30 | Парник — плоское K, не масштабируется с расстоянием | ✅ ИСПРАВЛЕНО | generate-planets.ts: парник = % от T_eq |
| G-31 | Имя планеты не содержит тип | ✅ ИСПРАВЛЕНО | generate-planets.ts: TYPE_NAMES в имени |
| G-32 | Независимая вариация M/L/T/R нарушает Стефана-Больцмана | ✅ ИСПРАВЛЕНО | generate-systems.ts: L из таблицы±15%, R из массы, T из L/R (Стефан-Больцман); спецтипы: T/R из диапазонов, L = R²×(T/T☉)⁴ |

---

## 2. Архитектура генерации

### 2.1 Оркестратор + 5 модулей (рефакторинг v3)

Генератор был рефакторинг из одного файла `generator.ts` (~1200 строк) в оркестратор + 5 модулей + общий контекст.

```
src/galaxy/
  index.ts              — экспорт generateGalaxy + типы
  generator.ts          — ОРКЕСТРАТОР (~118 строк)
  gen-context.ts        — общий контекст (genId, usedNames, resetGenContext)
  generate-positions.ts — генерация позиций (спираль, ядро, диск, ореол)
  generate-systems.ts   — генерация звёздных систем (звёзды, компаньоны, имена)
  generate-planets.ts   — генерация планет (тип, атмосфера, температура, жизнь)
  generate-resources.ts — генерация ресурсов (deposits, ultra-rare)
  generate-jump-points.ts — JP + связность
  hex-grid.ts           — гексагональная сетка (без изменений)
```

### 2.2 Граф зависимостей (DAG, без циклов)

```
generator.ts (оркестратор)
  ├── gen-context.ts
  ├── generate-positions.ts
  ├── generate-systems.ts
  │     ├── gen-context.ts
  │     └── generate-planets.ts
  │           ├── gen-context.ts
  │           ├── generate-resources.ts
  │           └── hex-grid.ts
  └── generate-jump-points.ts
        └── gen-context.ts
```

### 2.3 Цепочка вызовов (текущая)

```
generateGalaxy(config)                              — generator.ts
├── mainRng = new Xoshiro256(seed)
├── armRng = mainRng.derive('arms')
├── bulgeRng = mainRng.derive('bulge')
├── diskRng = mainRng.derive('disk')                ← G-05: ИСПОЛЬЗУЕТСЯ
├── haloRng = mainRng.derive('halo')
├── jpRng = mainRng.derive('jump_points')
│
├── generateSpiralPositions(cfg, armRng, bulgeRng, diskRng, haloRng)  — generate-positions.ts
│   ├── Ядро: ~15% (bulgeFraction=0.15)             ← G-14 fix
│   ├── Диск: ~20% (diskFraction=0.20)              ← G-05 fix
│   ├── Рукава: ~60% (остаток)
│   └── Ореол: ~5% (haloFraction из cfg)
│
├── for i in positions:
│   ├── systemRng = mainRng.derive(`system_${i}`)
│   └── generateSystem(pos, i, systemRng, cfg)      — generate-systems.ts
│       ├── selectBinaryType(rng) → BINARY_NONE/CLOSE/WIDE/TRIPLE
│       ├── weightedChoice(STAR_TYPES, STAR_WEIGHTS) → тип главной звезды
│       ├── generateSystemName(rng, index)
│       ├── createStar(systemId, name, starDef, rng.derive('star_primary'))
│       │   ├── G-32: mass = starDef.mass × (0.85-1.15)     ← ±15% вариация
│       │   ├── G-32: luminosity = starDef.luminosity × (0.85-1.15) ← ±15% (главная послед.)
│       │   ├── G-32: radius = M^0.8 (M<1) или M^0.57 (M≥1) ← из массы
│       │   └── G-32: temperature = 5778 × (L/R²)^0.25     ← Стефан-Больцман
│       ├── [selectCompanionStar + createStar] × 1-2  ← G-12: только main sequence
│       ├── totalPlanets = rng.nextInt(starDef.minPlanets, starDef.maxPlanets)
│       ├── planetCount = floor(totalPlanets × stabilityMult)
│       ├── asteroidFields с бонусами (G-17)         ← +1 за ГГ, +1 за двойную
│       └── for j in planetCount:
│           ├── planetRng = rng.derive(`planet_${j}`)
│           └── generatePlanet(systemId, j+1, name, primaryStar, binaryType, planetRng)
│               │                                   — generate-planets.ts
│               ├── hzCenter = sqrt(L/0.8)           ← G-27: масштабирование орбит
│               ├── orbitalRadius = hzCenter × ...    ← G-13: бинарные ограничения
│               ├── selectPlanetType(orbitalRadius, primaryStar, rng)
│               │   ├── HZ inner/outer + snow line    ← G-16: 4 зоны
│               │   └── 10% аномальная планета
│               ├── radiusKm = PLANET_TYPE_RADIUS[type]  ← G-04: из типа
│               ├── size = getSizeFromRadius(radiusKm)   ← G-06: из R⊕
│               ├── density = PLANET_DENSITY[type] + rng
│               ├── gravity = (radiusKm/6371) × (density/5.51)
│               ├── orbitalPeriod = 365.25 × sqrt(r³/M)
│               ├── atmosphere = generateAtmosphere(...)   ← G-02: ДО температуры
│               ├── temperature = calculatePlanetTemperature(star, r, planetDef, atmosphere, rng)
│               │   ├── T_eq = 278.5 × L^0.25 × r^(-0.5) ← G-18: 278.5
│               │   ├── greenhouseK = T_eq × (greenhousePercent/100) ← G-30: % от T_eq
│               │   └── typeModifierK (volcanic/ice/gas_giant/desert)
│               ├── life = generateLife(planetDef, atmosphere, temperature, rng)
│               │   ├── LIFE_LEVEL_WEIGHTS per-type     ← G-09 fix
│               │   ├── температура < -20 или > +80 → только microbes ← G-10 fix
│               │   ├── complex/simple → нужна standard/dense ← G-11 fix
│               │   └── toxic → только microbes (экстремофилы)  ← G-11 fix
│               ├── hexes = generateHexGrid(size, terrainWeights, rng.derive('hexes'))
│               ├── assignResourceDeposits(hexes, rng.derive('deposits'), type) — generate-resources.ts
│               ├── atmosphericSlots (только gas_giant, 6-12)
│               ├── orbitSlots = ORBIT_SLOTS_BY_SIZE[size]  ← G-15 fix
│               ├── aggregateResourceDeposits(hexes, type, rng.derive('ultra')) — generate-resources.ts
│               └── name = `${systemName} ${toRoman(orbit)} — ${typeName}` ← G-31 fix
│
├── generateJumpPoints(systems, jpRng, cfg)         — generate-jump-points.ts
└── ensureConnectivity(systems, jpRng, cfg)          — generate-jump-points.ts
```

### 2.4 PRNG — Xoshiro256

- `derive(name)`: 4 независимых хеша имени (FNV-1a + Murmur2 + Murmur3 c1/c2) XOR с основным состоянием — воспроизводимый дочерний генератор (G-24 fix)
- Каждый этап генерации использует свой `derive()`, что обеспечивает локальность изменений
- `weightedChoice(items, weights)` — взвешенный выбор с нормализацией
- `nextGaussian(mean, stddev)` — Box-Muller для нормального распределения

### 2.5 Общий контекст — gen-context.ts

```typescript
let nextId = 1;
export const usedNames = new Set<string>();

genId(prefix)       — генерация уникального ID с префиксом
resetGenContext()    — сброс контекста для воспроизводимости
```

Сбрасывается при каждой новой генерации через `resetGenContext()` в оркестраторе.

---

## 3. Типы планет — радиус, гравитация, размер сетки

### 3.1 Радиус: текущая реализация (G-04 fix)

**Радиус теперь берётся из `PLANET_TYPE_RADIUS[type]`** — единого диапазона на тип. Размер сетки **выводится** из радиуса через `getSizeFromRadius()`.

| Тип | Спец: радиус (км) | Код: PLANET_TYPE_RADIUS (км) | Δ |
|-----|-------------------|------------------------------|---|
| Rocky | 2 000 – 7 000 | **2 000 – 10 000** | 🟡 G-25: max расширен |
| Volcanic | 2 500 – 8 000 | 2 500 – 8 000 | ✅ |
| Ice | 1 500 – 6 000 | 1 500 – 6 000 | ✅ |
| Oceanic | 4 000 – 8 000 | **4 000 – 11 000** | 🟡 G-25: max расширен |
| Desert | 2 000 – 6 500 | 2 000 – 6 500 | ✅ |
| Gas Giant | 25 000 – 80 000 | 25 000 – 80 000 | ✅ |
| Dwarf | 500 – 2 000 | 500 – 2 000 | ✅ |

**🟡 G-25: Rocky max=10000 (спец: 7000), Oceanic max=11000 (спец: 8000).**
Осознанное отклонение: позволяет генерировать планеты размера "large" (91 гекс). Без расширения Rocky/Oceanic не могли бы иметь "Большая" сетку, т.к. при max 7000/8000 км R⊕<1.3 → всегда "Средняя".

### 3.2 Размер сетки (G-06 fix)

**Код:** `getSizeFromRadius(radiusKm)` — из R⊕ по формуле из 30-planets.md §2.1:
```
R < 0.3 → tiny (19)
R < 0.7 → small (37)
R < 1.3 → medium (61)
R < 2.0 → large (91)
R ≥ 2.0 → huge (127)
```
✅ Полностью соответствует спецификации.

**Следствие G-25:** Rocky при max=10000 км → R=1.57 → "large" (91 гекс). При спец max=7000 → R=1.10 → только "medium" (61 гекс). Аналогично Oceanic при max=11000 → R=1.73 → "large".

### 3.3 Гравитация

**Формула:** `gravity = (radiusKm / 6371) * (density / 5.51)` ✅ Верно по спец §2.2

**Плотность по типам:**

| Тип | Спец: плотность (г/см³) | Код: PLANET_DENSITY | Δ |
|-----|------------------------|---------------------|---|
| Rocky | 3.5 – 6.5 | 3.5 – 6.5 | ✅ |
| Volcanic | 4.0 – 7.0 | 4.0 – 7.0 | ✅ |
| Ice | 2.0 – 4.5 | 2.0 – 4.5 | ✅ |
| Oceanic | 3.5 – 5.5 | 3.5 – 5.5 | ✅ |
| Desert | 3.0 – 5.5 | 3.0 – 5.5 | ✅ |
| Gas Giant | 0.8 – 2.5 | **1.2 – 2.5** | 🟡 G-29: min поднят |
| Dwarf | 2.0 – 5.0 | 2.0 – 5.0 | ✅ |

**🟡 G-29: gas_giant density min=1.2 (спец: 0.8).**
Осознанное отклонение: гарантирует минимальную гравитацию ГГ ~0.9g (вместо ~0.4g при density=0.8 и R=25000/6371≈3.9 → g=3.9×0.8/5.51≈0.57). Это геймплейное решение — газовые гиганты с <0.5g выглядят нереалистично.

### 3.4 Резюме: радиус/гравитация/размер

Все старые баги (G-04, G-06) исправлены. Оставшиеся отклонения — осознанные (G-25, G-29), задокументированы в §13.

---

## 4. Температура планеты

### 4.1 Формула (G-02 + G-03 + G-18 + G-30 fix)

**Текущая реализация:**
```
T_eq = 278.5 × L^(1/4) × r^(-1/2)  [K]
greenhouseK = T_eq × (greenhousePercent / 100)  [K]    ← G-30: % от T_eq
T_final = T_eq + greenhouseK + typeModifierK - 273.15  [°C]
```

- ✅ G-03: Принимает реальный Star (с ±20% вариацией), не starDef
- ✅ G-02: Парниковый эффект на основе РЕАЛЬНОГО типа атмосферы (не свой roll)
- ✅ G-18: 278.5K (было 278)
- ✅ G-30: Парник = ПРОЦЕНТ от T_eq (масштабируется с расстоянием)

**Эквивалентность со спецификацией §2.3:**
```
Спец:  T = T_star × sqrt(R_star × 0.00465 / (2 × D_AU)) − 273.15
Код:   T_eq = 278.5 × L^0.25 × r^(-0.5)
```
Эти формулы **математически эквивалентны** (L = (R_star)² × (T_star/T_sun)⁴, но с упрощением). Код использует более прямую формулировку через светимость.

### 4.2 Парниковый эффект по типу атмосферы (G-30 fix)

**G-30 fix:** Парниковый эффект теперь = **ПРОЦЕНТ от T_eq**, а не плоское добавление в K.

Физическое обоснование: парниковый эффект захватывает ДОЛЮ исходящего излучения, которое пропорционально приходящему (равновесие). Поэтому парник автоматически масштабируется с расстоянием — далёкие планеты получают меньше нагрева от парника.

Старый баг: плоское +300K от CO₂ на 5 AU давало температуру +218°C (больше, чем у более близкой планеты), что физически невозможно.

| Тип атмосферы | Код: greenhousePercent | При 1 AU (T_eq=278K) | При 5 AU (T_eq=125K) |
|---------------|----------------------|---------------------|---------------------|
| none | 0% | +0K | +0K |
| thin | 3–10% | +8–28K | +4–13K |
| standard | 10–20% | +28–56K | +13–25K |
| dense | 20–45% | +56–125K | +25–56K |
| co2 | 30–60% | +84–167K | +38–75K |
| methane | 20–50% | +56–139K | +25–63K |
| toxic | 5–20% | +14–56K | +6–25K |
| inert | 2–7% | +6–19K | +3–9K |

**Проверка (Земля):** T_eq=255K, standard=10-20% → +26-51K. Реальность: +33K ✓

### 4.3 Модификаторы типа планеты (G-30 fix)

| Тип | Модификатор | Обоснование |
|-----|-------------|-------------|
| volcanic | +30..+100K | Геотермальный нагрев (внутренний источник, не зависит от звезды). Было +100-400K — слишком много |
| ice | -20..-50K | Высокое альбедо |
| gas_giant | +10..+30K | Внутренний нагрев (Кельвин-Гельмгольц). Было +20-60K |
| desert | -5..+10K | Низкое альбедо днём / быстрое охлаждение ночью |

Модификаторы остались плоскими (в K), т.к. это внутренние источники/альбедо, не зависящие от звезды.

---

## 5. Атмосфера — вероятности типов

### 5.1 Газовый гигант (G-08 fix)

| Тип | Спец §2.4 (абс.) | Код (условная) | Статус |
|-----|-------------------|----------------|--------|
| dense | 36% | 36% | ✅ |
| standard | 5% | 5% | ✅ |
| methane | 25% | 25% | ✅ |
| co2 | 25% | 25% | ✅ |
| inert | 5% | 5% | ✅ G-08 fix |
| toxic | 4% | 4% | ✅ G-08 fix |

**⚠️ Внимание:** §1.2.6 даёт ДРУГОЕ распределение: Dense=60%, Standard=10%, Methane=15%, CO₂=10%, Inert=5% (без toxic). См. §6.

### 5.2 Скалистая (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 40% | 40% | ✅ |
| standard | 35% | 35% | ✅ |
| dense | 7.5% | 7.5% | ✅ |
| toxic | 5% | 5% | ✅ |
| inert | 5% | 5% | ✅ |
| methane | 2.5% | 2.5% | ✅ |
| co2 | 5% | 5% | ✅ |

**⚠️ Внимание:** §1.2.1 даёт ДРУГОЕ распределение (4 типа: Thin=40%, Standard=35%, Dense=15%, Inert=10%). См. §6.

### 5.3 Вулканическая (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 10% | 10% | ✅ |
| standard | 5% | 5% | ✅ |
| dense | 20% | 20% | ✅ |
| toxic | 30% | 30% | ✅ |
| inert | 10% | 10% | ✅ |
| methane | 3.3% | 3.3% | ✅ |
| co2 | 21.7% | 21.7% | ✅ |

### 5.4 Ледяная (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 40% | 40% | ✅ |
| standard | 5% | 5% | ✅ |
| dense | 5% | 5% | ✅ |
| toxic | 5% | 5% | ✅ |
| inert | 15% | 15% | ✅ |
| methane | 10% | 10% | ✅ |
| co2 | 20% | 20% | ✅ |

### 5.5 Океаническая (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 14.1% | 14.1% | ✅ |
| standard | 55.3% | 55.3% | ✅ |
| dense | 20% | 20% | ✅ |
| toxic | 2.4% | 2.4% | ✅ |
| inert | 2.4% | 2.4% | ✅ |
| methane | 1.2% | 1.2% | ✅ |
| co2 | 4.7% | 4.7% | ✅ |

### 5.6 Пустынная (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 26.7% | 26.7% | ✅ |
| standard | 6.7% | 6.7% | ✅ |
| dense | 6.7% | 6.7% | ✅ |
| toxic | 13.3% | 13.3% | ✅ |
| inert | 13.3% | 13.3% | ✅ |
| methane | 13.3% | 13.3% | ✅ |
| co2 | 20% | 20% | ✅ |

### 5.7 Карликовая (G-07 fix)

| Тип | Спец §2.4 (усл.) | Код | Статус |
|-----|-------------------|-----|--------|
| thin | 50% | 50% | ✅ |
| standard | 10% | 10% | ✅ |
| inert | 30% | 30% | ✅ |
| co2 | 10% | 10% | ✅ |

### 5.8 Резюме по атмосфере

**Все атмосферные вероятности в коде теперь точно соответствуют §2.4** (условные вероятности пересчитаны из абсолютных). Однако внутри самой спецификации есть противоречия между §1.2 (описания типов) и §2.4 (таблица вероятностей). Код следует §2.4 как более детализированному источнику. См. §6.

---

## 6. Внутренние противоречия в спецификации

**Проблема:** В `docs/30-planets.md` §1.2 (описание каждого типа) и §2.4 (таблица вероятностей атмосферы) содержат **разные** распределения типов атмосферы. Код однозначно следует §2.4.

| Тип | §1.2: типы атмосферы | §2.4: типы атмосферы | Δ |
|-----|---------------------|---------------------|---|
| **Rocky** | Thin=40%, Standard=35%, Dense=15%, Inert=10% | Thin=40%, Std=35%, Dense=7.5%, Toxic=5%, Inert=5%, Methane=2.5%, CO₂=5% | §1.2: 4 типа, Dense=15%; §2.4: 7 типов, Dense=7.5% |
| **Volcanic** | Dense=30%, Toxic=45%, Inert=15%, CO₂=10% | Thin=10%, Std=5%, Dense=20%, Toxic=30%, Inert=10%, Methane=3.3%, CO₂=21.7% | §1.2: 4 типа; §2.4: 7 типов, совершенно другие % |
| **Ice** | Thin=50%, Inert=30%, CO₂=20% | Thin=40%, Std=5%, Dense=5%, Toxic=5%, Inert=15%, Methane=10%, CO₂=20% | §1.2: 3 типа, Inert=30%; §2.4: 7 типов, Inert=15% |
| **Oceanic** | Standard=55%, Dense=25%, Thin=15%, Inert=5% | Thin=14.1%, Std=55.3%, Dense=20%, Toxic=2.4%, Inert=2.4%, Methane=1.2%, CO₂=4.7% | §1.2: Dense=25%; §2.4: Dense=20% |
| **Desert** | Thin=40%, CO₂=35%, Toxic=15%, Inert=10% | Thin=26.7%, Std=6.7%, Dense=6.7%, Toxic=13.3%, Inert=13.3%, Methane=13.3%, CO₂=20% | §1.2: CO₂=35%; §2.4: CO₂=20% |
| **Gas Giant** | Dense=60%, Standard=10%, Methane=15%, CO₂=10%, Inert=5% | Dense=36%, Std=5%, Methane=25%, CO₂=25%, Inert=5%, Toxic=4% | §1.2: Dense=60%; §2.4: Dense=36%. Toxic отсутствует в §1.2 |
| **Dwarf** | Thin=70%, Inert=30% | Thin=50%, Std=10%, Inert=30%, CO₂=10% | §1.2: Thin=70%; §2.4: Thin=50% |

**Рекомендация:** Обновить §1.2, чтобы он соответствовал §2.4, или добавить в §1.2 пометку «упрощённое распределение, точное см. §2.4».

---

## 7. Жизнь на планетах

### 7.1 lifeChance и LIFE_LEVEL_WEIGHTS (G-01 + G-09 fix)

**Код теперь использует `LIFE_LEVEL_WEIGHTS`** — per-type взвешенный выбор по 5 уровням: [none, microbes, plants, simple, complex].

| Тип | Спец: нет/микробы/растения/простая/сложная | Код: LIFE_LEVEL_WEIGHTS | Статус |
|-----|---------------------------------------------|-------------------------|--------|
| Rocky | 50/30/15/5/0 | [50, 30, 15, 5, 0] | ✅ |
| Volcanic | 95/5/0/0/0 | [95, 5, 0, 0, 0] | ✅ |
| Ice | 85/15/0/0/0 | [85, 15, 0, 0, 0] | ✅ |
| Oceanic | 5/10/30/35/20 | [5, 10, 30, 35, 20] | ✅ |
| Desert | 75/20/5/0/0 | [75, 20, 5, 0, 0] | ✅ |
| Gas Giant | 100/0/0/0/0 | [100, 0, 0, 0, 0] | ✅ |
| Dwarf | 98/2/0/0/0 | [98, 2, 0, 0, 0] | ✅ |

**Поле `lifeChance` в PLANET_TYPES** обновлено (G-01), но **фактически не используется** `generateLife()` — функция работает через `LIFE_LEVEL_WEIGHTS`. Поле можно считать справочным/legacy.

### 7.2 Условия для жизни (G-10 + G-11 fix)

**Текущие проверки в generateLife():**
1. ❌ Газовый гигант → нет жизни ✅
2. ❌ Без атмосферы (none) → нет жизни ✅
3. ⚠️ Температура < -20°C или > +80°C → только microbes или none ✅ (G-10)
4. ⚠️ complex/simple жизнь → нужна standard/dense атмосфера ✅ (G-11)
5. ⚠️ toxic атмосфера → только microbes (экстремофилы) ✅ (G-11)

**Не проверяется (из спец §2.5):**
- 🟡 Жидкая вода: наличие океанов или подлёдных вод — не проверяется
- 🟡 Магнитное поле: желательно для сложной жизни — не реализовано (нет поля)

### 7.3 Резюме

Все критические баги жизни исправлены. Оставшиеся ограничения (жидкая вода, магнитное поле) зависят от нереализованных полей (magneticField) и не могут быть добавлены без расширения модели Planet.

---

## 8. Ресурсы

### 8.1 Профильные элементы: спецификация vs код

| Тип | Спец: ключевые ресурсы | Код: PROFILE_ELEMENTS | Δ |
|-----|----------------------|----------------------|---|
| Rocky | Si, Al, O, C, **Ca**, Fe | Fe, Si, Al, C, O | ❌ Ca отсутствует в ELEMENTS |
| Volcanic | Fe, Ti, Cr, Ni, **W**, S, V, **Mo** | Fe, Ti, Cr, Ni, S, V | ❌ W→RARE, Mo отсутствует |
| Ice | H, O, He, N, **Ne** | H, O, N, He | ❌ Ne отсутствует |
| Oceanic | H, O, **Na, Cl, Mg, Ca** | H, O, N, C, S | ❌ Na, Cl, Mg, Ca отсутствуют; N, C, S — не по спец. |
| Desert | Si, Fe, Al, Ti, **Rare Earth** | Si, Fe, Al, Ti | ❌ Редкоземельные отсутствуют |
| Gas Giant | H, He, C, N, CH₄, NH₃ | H, He, C, N | 🟡 CH₄/NH₃ — соединения, не элементы |
| Dwarf | Fe, Ni, Si, C, **Pt, Ir** | Fe, Si, C, Ni | ❌ Pt→RARE, Ir отсутствует |

**Причина:** ELEMENTS содержит 22 элемента (MVP), спецификация упоминает 57 элементов (см. `32-mendeleev.md`). Это **не баг генератора**, а ограничание MVP — полная реализация покроет все 57.

### 8.2 Система ресурсов

Трёхтирная система (profile/rare/ultra_rare) работает корректно. Каждая планета имеет ВСЕ 22 элемента с разными количествами. Газовые гиганты получают элементы напрямую (без гексов).

**Категории множителей (generate-resources.ts):**

| Тип | structural | fuel | alloy | electronics | chemical | energy | rare | light |
|-----|-----------|------|-------|-------------|----------|--------|------|-------|
| rocky | 2.5 | 0.5 | 1.5 | 0.8 | 0.6 | 0.5 | 0.3 | 0.5 |
| volcanic | 1.8 | 0.3 | 2.5 | 0.6 | 1.2 | 2.0 | 0.8 | 0.2 |
| ice | 0.4 | 2.0 | 0.3 | 0.2 | 1.8 | 0.2 | 0.15 | 1.0 |
| oceanic | 0.7 | 1.0 | 0.5 | 0.5 | 1.8 | 0.3 | 0.2 | 0.6 |
| desert | 1.2 | 0.2 | 1.0 | 0.6 | 0.3 | 0.6 | 0.5 | 0.3 |
| gas_giant | 0.1 | 3.0 | 0.1 | 0.1 | 2.5 | 0.1 | 0.1 | 1.5 |
| dwarf | 0.6 | 0.3 | 0.4 | 0.2 | 0.3 | 0.2 | 0.15 | 0.3 |

---

## 9. Орбитальные и атмосферные слоты

### 9.1 Орбитальные слоты (G-15 fix)

**Код:** Не-ГГ планеты используют `ORBIT_SLOTS_BY_SIZE[size]` — фиксированное количество по размеру.

| Размер | Спец: орбитальные слоты | Код: ORBIT_SLOTS_BY_SIZE | Статус |
|--------|------------------------|-------------------------|--------|
| tiny | 3 | 3 | ✅ |
| small | 4 | 4 | ✅ |
| medium | 5 | 5 | ✅ |
| large | 5 | 5 | ✅ |
| huge | 6 | 6 | ✅ |
| gas_giant | 6–12 | 6–12 | ✅ |

**G-23 fix:** Dwarf всегда tiny=3 слота (было 2-4). ✅

### 9.2 Атмосферные слоты

Gas giant: 6–12 ✅. Остальные: 0 ✅.

---

## 10. Звёзды и планетные системы

### 10.1 Типы звёзд

12 типов, параметры из 02-stars.md ✅. Веса из документации ✅.

### 10.2 Вариация параметров (G-32 fix)

**Код (G-32 fix):** Следует алгоритму из 02-stars.md §6, Шаг 4:

**Главная последовательность:**
1. Масса: ±15% от табличного среднего
2. Светимость: ±15% от табличного среднего (сохраняет правильные средние для O/B)
3. Радиус: R = M^0.8 (M<1) или R = M^0.57 (M≥1) — из массы
4. Температура: T = 5778 × (L/R²)^0.25 — закон Стефана-Больцмана

Это гарантирует L = R² × (T/T☉)⁴ — физическую согласованность всех параметров.

**Специальные типы:**
1. T и R выбираются из документированных диапазонов (02-stars.md §2.1)
2. L вычисляется из T и R: L = R² × (T/5778)⁴ — Стефан-Больцман
3. Для WD: T=8000-40000K, R=0.008-0.02 R☉ → L=0.0002-0.9 L☉ (шире чем 0.001-0.01 в таблице, но физически корректно)
4. Для RG: T=3000-5000K, R=10-200 R☉ → L вычисляется
5. Для NS/PULSAR: L ≈ 0 в оптике, T косметическая
6. Для BH: L=0, T=0

**Проверка (Земля):** M=1.0, L=1.0, R=1.0 → T=5778K ✓ T_eq(1AU)=5.5°C ✓

### 10.3 Двойные/тройные системы

NONE=60%, CLOSE=20%, WIDE=15%, TRIPLE=5% ✅

### 10.4 Спутник звезды (G-12 fix)

**Код:** `selectCompanionStar` выбирает только из главной последовательности (7 типов, индексы 0-6). ✅
Если главная звезда — специальный тип, компаньон выбирается случайно из main sequence.

### 10.5 Stability multiplier

Одиночная=1.0, Широкая=0.9, Близкая=0.6, Тройная=0.5 ✅

### 10.6 Орбитальный радиус (G-13 + G-27 fix)

**Текущая формула:**
```
hzCenter = sqrt(L / 0.8)
orbitalScale = min(hzCenter, 5.0)
orbitalRadius = orbitalScale × (0.3 + orbit × (0.5 + rng × 0.3))
```

- ✅ G-27: Масштабирование от HZ-центра — планеты M/K карликов попадают в HZ
- ✅ G-13: BINARY_CLOSE → min 1.0 AU; BINARY_WIDE → max 30 AU

**⚠️ Не реализовано из спец §3.4:**
- Близкая двойная: планеты только дальше 3×d_binary от центра масс
- Широкая двойная: планеты ограничены 0.3×d (где d — расстояние между компонентами)

Текущая реализация использует фиксированные ограничения (1 AU / 30 AU) вместо формул из спец.

### 10.7 Астероидные поля (G-17 fix)

- ✅ +1 если есть газовый гигант (10% bonus)
- ✅ +1 если двойная/тройная система (15% bonus)
- 🟡 Упрощение: бонус = +1 (а не процентная вероятность +10%/+15%)

### 10.8 Имена систем (L-02)

**Формат:** `Greek Constellation[-N]`
- Greek: Alpha, Beta, Gamma, ... Omega (24 слова)
- Constellation: Centauri, Cygni, Draconis, ... Virginis (15 слов)
- Суффикс `-N` добавляется при коллизии (до 100 попыток)
- Уникальность гарантируется через `usedNames` в gen-context.ts

---

## 11. Спиральная структура галактики

### 11.1 Распределение (G-05 + G-14 fix)

| Область | Спец | Код | Статус |
|---------|------|-----|--------|
| Ядро (bulge) | ~15% | 15% | ✅ |
| Диск (disk) | ~20% | 20% | ✅ |
| Рукава (arms) | ~60% | ~60% | ✅ |
| Ореол (halo) | ~5% | 5% | ✅ |

### 11.2 Алгоритм

**Спец (§1.1):** r(θ) = a × e^(b×θ) — логарифмическая спираль (угол → радиус)
**Код:** dist → spiralAngle + гауссово смещение (радиус → угол)

🟡 Обратный порядок параметризации, но визуальный результат аналогичен. Не влияет на геймплей.

### 11.3 diskFraction в конфиге

**Внимание:** `GalaxyGenConfig.diskFraction = 0.2` существует, но в `generateSpiralPositions` используется **захардкоженное** `diskFraction = 0.20`. Конфиг-параметр `cfg.diskFraction` не передаётся в `PositionsConfig` и игнорируется для распределения.

🟡 Потенциальная путаница: diskFraction определён в конфиге, но не используется в generateSpiralPositions.

---

## 12. Jump Points и связность

### 12.1 Генерация JP

- ✅ Расстояние ≤ max_jp_distance
- ✅ Max 6 JP на систему
- ✅ Bidirectional
- ❌ Угол выравнивания с рукавом — не реализовано (опционально по спец)
- ❌ Макро-объекты могут блокировать JP — не реализовано (нет макро-объектов)

### 12.2 Связность

BFS + мосты ✅. Алгоритм корректен.

---

## 13. Осознанные отклонения от спецификации

Следующие отклонения являются **намеренными решениями**, задокументированными в комментариях кода:

| ID | Отклонение | Причина | Файл |
|----|-----------|---------|------|
| G-25 | Rocky max radius: 10000 (спец: 7000) | Позволяет размер "large" (91 гекс). Без этого Rocky ≤ medium (61 гекс) | planet-types.ts:30 |
| G-25 | Oceanic max radius: 11000 (спец: 8000) | Позволяет размер "large" (91 гекс). Без этого Oceanic ≤ medium (61 гекс) | planet-types.ts:33 |
| G-27 | Орбитальный радиус масштабируется от hzCenter | Без этого планеты M/K карликов все попадают в outer zone (далеко от HZ) | generate-planets.ts:365 |
| G-29 | Gas giant density min: 1.2 (спец: 0.8) | Гарантирует мин. гравитацию ГГ ~0.9g (вместо ~0.4g) | planet-types.ts:20 |
| G-19 | Вариация звёзд ±20% вместо subclass-based интерполяции | Упрощение для MVP | generate-systems.ts:68-71 |
| G-30 | Парник = % от T_eq (было плоское K) | Физически корректно: парник пропорционален приходящему излучению | generate-planets.ts:148-159 |

---

## 14. Нереализованные возможности из спецификации

### 14.1 Из 30-planets.md

| Возможность | Раздел | Статус | Приоритет |
|-------------|--------|--------|-----------|
| Магнитное поле | §2.7 | ❌ Не реализовано | Medium (влияет на жизнь) |
| Сложные органические соединения | §2.6 | ❌ Не реализовано | Low |
| Спутники планет | §1.3 | ❌ Не реализовано | Medium |
| Габитабельность | §1.1 | ❌ Не реализовано (нет поля) | High |
| Composition атмосферы | §2.4 | 🟡 Поле есть, всегда `[]` | Low |
| Бонусы/штрафы гравитации | §2.2 | ❌ Не применяются | Medium |
| Бонусы/штрафы температуры | §2.3 | ❌ Не применяются | Medium |

### 14.2 Из 02-stars.md

| Возможность | Раздел | Статус | Приоритет |
|-------------|--------|--------|-----------|
| Кометные облака (Оорт) | §5 | ❌ | Low |
| Линия снега для типов планет | §3.3 | ✅ Реализовано (G-16) | — |
| Множитель вероятности ГГ по типу звезды | §3.3 | ❌ | Medium |
| Ресурсы астероидных полей | §4.3 | ❌ (только количество) | Low |
| Косметический подкласс звезды | §1.3 | ❌ | Low |

### 14.3 Из 01-galaxy.md

| Возможность | Раздел | Статус | Приоритет |
|-------------|--------|--------|-----------|
| Туманности | §3.1 | ❌ | Low |
| Газовые скопления | §3.2 | ❌ | Low |
| Межзвёздные астероидные пояса | §3.3 | ❌ | Low |
| Пустоты | §3.4 | ❌ | Low |
| Реликты Предтеч | §3.5 | ❌ | Medium |
| Варп-двигатели | §2.4.2 | ❌ | Post-MVP |
| Альтернативные формы | §1.4 | ❌ | Post-MVP |
| k-d tree для пространственного индекса | §2.3 | ❌ | Medium (оптимизация) |

### 14.4 Недостающие элементы

| Элемент | Источник | Статус |
|---------|----------|--------|
| Na, Cl, Mg, Ca, Ne, Mo, Ir, Rare Earth | 30-planets.md §1.2 | ❌ Не в ELEMENTS (MVP: 22 элемента) |

---

## 15. Сводная таблица текущих расхождений

### 🟢 Исправлено (было 🔴/🟡 в v1/v2)

Все G-01..G-18, G-24, G-30, G-31 исправлены. См. §1.

### 🟡 Осознанные отклонения (не баги, а дизайнерские решения)

| # | ID | Описание | Файл |
|---|-----|----------|------|
| 1 | G-25 | Rocky/Oceanic max radius расширен для "large" сетки | planet-types.ts:30,33 |
| 2 | G-27 | Орбитальный радиус масштабируется от hzCenter | generate-planets.ts:365 |
| 3 | G-29 | Gas giant density min=1.2 для мин. гравитации ~0.9g | planet-types.ts:20 |
| 4 | G-19 | Вариация звёзд: ±15% для M и L, R из массы, T из Стефана-Больцмана | generate-systems.ts:108-184 |
| 5 | G-20 | Позиции систем: радиус→угол вместо угол→радиус | generate-positions.ts:70-85 |
| 6 | G-30 | Парник = % от T_eq (было плоское K, немонотонная температура) | generate-planets.ts:148-159 |

### 🔴 Внутренние противоречия спецификации

| # | Описание | Документ |
|---|----------|----------|
| 6 | §1.2 Rocky: 4 типа атмосферы vs §2.4: 7 типов, разные % | 30-planets.md |
| 7 | §1.2 Volcanic: 4 типа vs §2.4: 7 типов, совершенно разные % | 30-planets.md |
| 8 | §1.2 Ice: 3 типа vs §2.4: 7 типов, Thin 50% vs 40% | 30-planets.md |
| 9 | §1.2 Oceanic: 4 типа vs §2.4: 7 типов, Dense 25% vs 20% | 30-planets.md |
| 10 | §1.2 Desert: 4 типа vs §2.4: 7 типов, CO₂ 35% vs 20% | 30-planets.md |
| 11 | §1.2 Gas Giant: 5 типов vs §2.4: 6 типов, Dense 60% vs 36% | 30-planets.md |
| 12 | §1.2 Dwarf: 2 типа vs §2.4: 4 типа, Thin 70% vs 50% | 30-planets.md |

**Код следует §2.4** как более детализированному источнику. Рекомендуется обновить §1.2.

### 🟡 Мелкие расхождения

| # | Описание | Файл |
|---|----------|------|
| 13 | diskFraction в конфиге существует, но не передаётся в PositionsConfig (захардкожено 0.20) | generator.ts:63; generate-positions.ts:40 |
| 14 | lifeChance в PLANET_TYPES не используется generateLife() (используется LIFE_LEVEL_WEIGHTS) | planet-types.ts:101 etc. |
| 15 | size/hexCount в PLANET_TYPES де-факто не используются (размер из getSizeFromRadius, hexCount из SIZE_HEX_COUNT) | planet-types.ts |
| 16 | Орбитальный радиус для двойных: фиксированные ограничения вместо формул из спец §3.4 | generate-planets.ts:369-373 |
| 17 | Астероидные поля: бонус +1 вместо процентной вероятности +10%/+15% | generate-systems.ts:132-138 |
| 18 | Composition атмосферы всегда `[]` | generate-planets.ts:182,192 |

### ⬜ Нереализованные функции (из спецификации, но не в коде)

См. §14. Всего 24 позиции (7 планетных, 5 звёздных, 8 галактических, 4 элементных).

---

## Приложение A: Карта файлов (текущая архитектура)

### generator.ts — ОРКЕСТРАТОР (~118 строк)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-17      Комментарий: описание оркестратора + 5 модулей
19-24     Импорты
26-67     GalaxyGenConfig + DEFAULT_CONFIG
69-118    generateGalaxy() — оркестратор
```

### gen-context.ts (~21 строка)

```
Строки    Функция / блок
────────  ────────────────────────────────
7-10      nextId, usedNames
12-15     genId(prefix)
17-21     resetGenContext()
```

### generate-positions.ts (~98 строк)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-9       Комментарий (G-05/G-14)
11-22     PositionsConfig interface
24-98     generateSpiralPositions()
            ├── bulge ~15%
            ├── disk ~20%
            ├── arms ~60%
            └── halo ~5%
```

### generate-systems.ts (~230 строк)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-4       Комментарий
6-10      Импорты
12-14     MAIN_SEQUENCE_STAR_TYPES (G-12)
16-18     GREEK, CONSTELLATIONS пулы имён
20-33     generateSystemName() (L-02)
35-42     selectBinaryType()
44-59     selectCompanionStar() — G-12: main sequence only
61-106    G-32 fix: SPECIAL_STAR_RANGES, MAIN_SEQUENCE_TYPES,
            stefanBoltzmannLuminosity(), stefanBoltzmannTemperature()
108-184   createStar() — G-32: L из таблицы±15%, R из массы, T из L/R (Стефан-Больцман)
186-260   generateSystem()
            ├── звёзды (одиночная/двойная/тройная)
            ├── планеты (через generatePlanet)
            └── asteroidFields (G-17)
```

### generate-planets.ts (~472 строки)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-14      Комментарий (физика: формулы)
16-21     Импорты
23-38     toRoman()
40-118    selectPlanetType() — G-16: 4 зоны + snow line
120-173   calculatePlanetTemperature() — G-02,G-03,G-18,G-30
175-271   Атмосфера:
            ├── generateAtmosphere()
            ├── selectGasGiantAtmosphereType() — G-08
            ├── selectAtmosphereType() — G-07: точные условные вероятности
            └── getAtmospherePressure()
273-342   generateLife() — G-01,G-09,G-10,G-11
344-472   generatePlanet() — G-02,G-03,G-04,G-06,G-13,G-15,G-27,G-31
```

### generate-resources.ts (~205 строк)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-8       Комментарий (трёхтирная система)
10-13     Импорты
15-24     CATEGORY_MULTIPLIERS (7 типов × 8 категорий)
26-104    assignResourceDeposits() — залежи на гексах
106-205   aggregateResourceDeposits() — агрегация + ультраредкие
```

### generate-jump-points.ts (~129 строк)

```
Строки    Функция / блок
────────  ────────────────────────────────
1-4       Комментарий (P1-23)
6-8       Импорты
10-13     JumpPointsConfig interface
14-66     generateJumpPoints()
67-129    ensureConnectivity() — BFS + мосты
```

### planet-types.ts (~264 строки) — данные

```
Строки    Данные
────────  ────────────────────────────────
13-22     PLANET_DENSITY — ✅ совпадает со спец (кроме G-29: gas_giant min)
24-37     PLANET_TYPE_RADIUS — G-04 fix (радиус из типа); G-25: rocky/oceanic max расширен
39-50     getSizeFromRadius() — G-06 fix (из R⊕)
52-62     ORBIT_SLOTS_BY_SIZE — G-15/G-23 fix (по размеру, не типу)
64-77     PROFILE_ELEMENTS — 🟡 частично: не все элементы в ELEMENTS
79-83     RARE_ELEMENTS
85-90     ULTRA_RARE_ELEMENTS
92-170    PLANET_TYPES[] — G-01: lifeChance исправлены
172       PLANET_TYPE_MAP
174-184   SIZE_HEX_COUNT — ✅
186-210   ALL_TERRAINS, TERRAIN_COLORS, TERRAIN_NAMES, SIZE_NAMES, TYPE_NAMES
230-234   GAS_GIANT_ATMOSPHERE_SLOTS — ✅
236-249   ORBIT_SLOTS — используется только для gas_giant
251-264   LIFE_LEVEL_WEIGHTS — G-09: per-type из спецификации
```

---

## Приложение B: Схема генерации (согласовано с пользователем)

1. Задаётся количество звёзд (systemCount)
2. Генерируются «гнёзда» — позиции согласно схеме галактики (ядро/рукава/диск/ореол)
3. Генерируется набор звёзд по правилам распределения → размещаются в гнёзда
4. Для каждой звезды по классу генерируются планеты
5. Генерация ультра-редких ресурсов + распределение по планетам
6. Финализация (JP, связность, стартовая система)
