...
> **Дата обновления:** 2026-05-04 — интеграция с [33-chemistry.md(./33-chemistry.md), добавление химического характера как ключевого свойства элемента  
> **Назначение:** Документ архитектуры кода — описание модульной системы ресурсов SpaceGame: как добавлять новые элементы/руды, как они автоматически интегрируются в производственные цепочки, и как правила химии (33-chemistry.md) управляют автогенерацией  
> **Зависимости:** [33-chemistry.md(./33-chemistry.md) (правила химических взаимодействий), [m[32-mendeleev.md(./32-mendeleev.md) (химическая таблица элементов), [34-ores.md](./34-ores.md) (расчёты руд)
...
- **Новая руда** генерируется **автоматически** по правилам химии из [33-chemistry.md(./33-chemistry.md) — на основе химического характера элемента и его степени окисления.
- **Ни одна функция** не требует ручного обновления при расширении таблицы элементов или руд.

### 1.2 Роль 33-chemistry.md

Документ [33-chemistry.md(./33-chemistry.md) — это **ядро системы модульности**. Он определяет:

| Вопрос | Ответ даёт |
|--------|-----------|
| Какую руду образует элемент? | 33-chemistry.md §4 — правила образования руд по химическому характеру |
| Какое здание добывает руду? | 33-chemistry.md §5 — матрица здание → тип руды |
| Какая переработка нужна? | 33-chemistry.md §6 — правила назначения переработки |
| Сколько энергии и времени? | 33-chemistry.md §7 — расчёт энергозатрат |
| Может ли элемент быть самородком? | 33-chemistry.md §8 — самородные элементы |
| Атмосферное/ледяное соединение? | 33-chemistry.md §9 — атмосферные и ледяные соединения |
...
```
            ...
            ┌───────────┼───────────┐
            ▼           ▼           ▼
      32-mendeleev.md   34-ores.md   modularity.md
...
Добавление нового элемента состоит из **4 шагов**, из которых шаги 2–3 выполняются автоматически по правилам [33-chemistry.md(./33-chemistry.md):

| Шаг | Действие | Ручной / Авто |
|-----|----------|---------------|
| 1 | Добавить элемент в таблицу `ELEMENTS` | **Ручной** |
| 2 | Определить химический характер и руду по правилам | **Авто** (по 33-chemistry.md) |
...
> **Новое поле `chemicalCharacter`** (планируемое): определяет химический характер элемента по классификации [33-chemistry.md(./33-chemistry.md) §2. Позволяет системе автоматически генерировать руду.

> **Новое поле `oxidationState`** (планируемое): типичная степень окисления элемента в руде. Определяет формулу минерала-прототипа по [33-chemistry.md(./33-chemistry.md) §3.

В `docs/32-mendeleev.md` добавить описание элемента в соответствующую категорию с:
- Символом, названием, атомным номером и массой
- Категорией в игре
- Химическим характером (по 33-chemistry.md §2.2)
- Степенью окисления (по 33-chemistry.md §3)
...
По алгоритму [33-chemistry.md(./33-chemistry.md) §11:

1. **Определить химический характер** → `reactive_metal` (для Ga)
2. **Определить тип руды и здание добычи** → `metal_ore`, `mine` (по 33-chemistry.md §5)
3. **Выбрать формулу минерала-прототипа** → Ga₂O₃ (по 33-chemistry.md §4.1, степень +3)
4. **Рассчитать молярную массу** → M(Ga₂O₃) = 2×69.7 + 3×16.0 = 187.4 г/моль
5. **Рассчитать выход** → Ga ≈ 7.4, O ≈ 2.6 (по 33-chemistry.md §7)
6. **Назначить переработку** → `processor`, мин. уровень 2 (по 33-chemistry.md §6)
7. **Рассчитать энергозатраты** → энергия 4, время 200 (по 33-chemistry.md §7)
8. **Определить самородный шанс** → 0 (по 33-chemistry.md §8)
...
В `docs/34-ores.md` добавить расчёт руды с полным разбором (как в 34-ores.md §3.1–3.3).
...
|----------|--------------|
| Определить химический характер элемента | Разработчик (по 33-chemistry.md §2) |
| Выбрать формулу минерала-прототипа | Автоматически (по 33-chemistry.md §4) |
| Рассчитать молярную массу и выход | Автоматически (по формулам) |
| Назначить здание добычи | Автоматически (по 33-chemistry.md §5) |
| Назначить здание переработки | Автоматически (по 33-chemistry.md §6) |
| Рассчитать энергозатраты | Автоматически (по 33-chemistry.md §7) |
| Определить самородный шанс | Автоматически (по 33-chemistry.md §8) |
...
| Поле | Тип | Описание |
|------|-----|----------|
| `chemicalCharacter` | `ChemicalCharacter` | Химический характер — определяет тип руды и здание добычи (см. [33-chemistry.md(./33-chemistry.md) §2) |
| `oxidationState` | `number` | Типичная степень окисления в руде — определяет формулу минерала (см. [33-chemistry.md(./33-chemistry.md) §3) |
| `rarity` | `'abundant'` \| `'common'` \| `'rare'` \| `'ultra_rare'` | Редкость — поправочный коэффициент для энергозатрат (см. [33-chemistry.md(./33-chemistry.md) §7.3) |
...
| Документ | Связь |
|----------|-------|
| [33-chemistry.md(./33-chemistry.md) | **Ядро модульности** — правила химических взаимодействий, автогенерация руд, алгоритм добавления элемента |
| [m[32-mendeleev.md(./32-mendeleev.md) | Список элементов, атомные массы, химические характеры — входные данные |
| [34-ores.md](./34-ores.md) | Конкретные руды с расчётами — результат применения правил 33-chemistry.md |
| [40-buildings.md](./40-buildings.md) | Здания добычи и переработки — соответствие ID зданий в коде и в документации |
| [00-ARCHITECTURE.md](./00-ARCHITECTURE.md) | Общая архитектура экономики (§3.1) |

---

*Документ обновлён 2026-05-04 (v2.0). Интегрирован с [33-chemistry.md(./33-chemistry.md). Согласовано с `src/data/elements.ts` (58 элементов), `src/data/processing-chains.ts` (33+18+3 руды, 11 газов, 5 льдов).*
