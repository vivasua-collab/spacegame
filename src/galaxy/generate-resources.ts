 ...
 * не трогается — повторный вызов не дублирует залежи. Сейвы v1/v2 имеют
 * запечённые залежи и помечаются depositsMaterialized при загрузке
 * (migrateLegacyDepositFlags), поэтому тоже безопасны.
 *
