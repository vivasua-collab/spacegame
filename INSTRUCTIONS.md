...
- **Формат `state`:** компактный JSON с маркером `fmt: 3` в корне —
  ленивые залежи (в сейве только гексы материализованных/
  колонизированных тел; истощённые кортежи не пишутся), словарь id
  `galaxy.dict`, кортежи-индексы.
  - **R-30:** v3 — единственный поддерживаемый формат. Сейвы v1/v2
    (fmt:2 / без маркера) при загрузке отклоняются явной ошибкой —
    декодеры-заглушки удалены, старые сейвы стёрты владельцем.
...
## Текущее состояние (обновлено 2026-09-03, R-27)

DEV-сервер запущен командой `bun run dev` (порт 3000, лог — `dev.log` через
`tee`). Этот экземпляр переживает завершение Bash-сессий (запущен из
персистентной сессии). Если сервер упал — используйте double-fork ниже.

Быстрая проверка живости (одной командой):
```bash
...
Лог большой — читать только хвост (`tail`), не весь файл.
