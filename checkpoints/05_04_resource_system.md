...
- ⏳ Отобразить цепочку руда → элемент в UI (какому зданию перерабатывать, сколько выходит)

### Качество кода
- ⏳ DRY: CATEGORY_LABELS определён дважды (resource-panel.tsx + planet-view.tsx) — вынести в общую константу

## Проблемы
- processing-chains.ts дублирует данные с chemistry-generator.ts — нужна миграция

## Следующие шаги
1. Создать UI отображения цепочки переработки (руда → здание → чистый элемент)
2. Мигрировать на BakedGalaxyModel как единый источник данных о рудах
